...
impl Zobrist {
    ...
    fn new() -> Self {
        ...
        let mut zobrist = Zobrist {
            ...
            en_passant: [rng.random(), rng.random(), rng.random(), rng.random(), rng.random(), rng.random(), rng.random(), rng.random()],
        };
...
impl ChessBoard {
    ...
    fn zobrist_hash(&self, side_to_move: Color) -> u64 {
        ...
        if self.castling_rights.white_queenside { hash ^= self.zobrist.castling_rights[0]; }
        if self.castling_rights.white_kingside { hash ^= self.zobrist.castling_rights[1]; }
        if self.castling_rights.black_queenside { hash ^= self.zobrist.castling_rights[2]; }
        if self.castling_rights.black_kingside { hash ^= self.zobrist.castling_rights[3]; }
    ...
    fn update_hash(&mut self, mv: &Move) {
        let from_square = self.map_to_standard_chess_board(mv.from) as usize;
        let to_square = self.map_to_standard_chess_board(mv.to) as usize;
        ...
        if let Some(castling) = &mv.castling {
            let rook_from = self.map_to_standard_chess_board(castling.rook_from) as usize;
            let rook_to = self.map_to_standard_chess_board(castling.rook_to) as usize;
        ...
        if mv.en_passant {
            let capture_square = if mv.piece.is_white() {
                self.map_to_standard_chess_board(mv.to - self.board_width) as usize
            } else {
                self.map_to_standard_chess_board(mv.to + self.board_width) as usize
            };
...
mod chess_board_tests {
    ...
    fn assert_board_states_equal(b1: &ChessBoard, b2: &ChessBoard, msg: &str) {
        ...
        assert_eq!(b1.castling_rights, b2.castling_rights, "{}: Castling rights mismatch", msg);
        assert_eq!(b1.en_passant_target, b2.en_passant_target, "{}: En passant target mismatch", msg);
        ...
        for square in 0..64 {
            ...
            assert_eq!(piece1, piece2, "{}: Piece mismatch at square {}", msg, square);
        }
    }
    ...
    fn test_make_unmake_move() {
        ...
        game.set_fen_position("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1");
...
mod zobrist_tests {
    ...
    fn create_test_board() -> ChessBoard {
        // Helper to create a fresh board for testing
        let mut game = GameState::default();
    ...
    fn test_hash_changes_with_side_to_move() {
        ...
        assert_ne!(white_hash, black_hash, "Hash should change with side to move");

        // Test that XORing side_to_move flips correctly
        let side_to_move_key = board.zobrist.side_to_move;
        assert_eq!(white_hash ^ side_to_move_key, black_hash,
    ...
    fn test_pawn_move_hash_update() {
        ...
        assert_ne!(initial_hash, after_move_hash, "Hash should change after pawn move");
    ...
    fn test_capture_hash_update() {
        ...
        game.set_fen_position("r1bqkbnr/1ppp1ppp/p1n5/1B2p3/4P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 0 1");
        ...
        assert_ne!(initial_hash, after_capture_hash, "Hash should change after capture");

        // Test unmake
        board.unmake_move(&mv);
        assert_eq!(initial_hash, board.hash, "Hash should restore after unmake capture");
    }

    #[test]
    fn test_castling_rights_loss_hash() {
        let mut game = GameState::default();
        game.set_fen_position("r3k2r/pp2pppp/8/8/8/8/PPP2PPP/R3K2R w KQkq - 0 1");
        ...
        assert_ne!(initial_hash, after_king_move_hash,
    ...
    fn test_rook_move_castling_rights_hash() {
        ...
        assert_ne!(initial_hash, after_rook_move_hash,
    ...
    fn test_castling_move_hash() {
        ...
        assert_ne!(initial_hash, after_castling_hash,
    ...
    fn test_en_passant_hash() {
        let mut game = GameState::default();
        game.set_fen_position("rnbqkbnr/pp2pppp/8/2ppP3/8/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 1");
        ...
        assert_ne!(initial_hash, after_ep_hash,
    ...
    fn test_promotion_hash() {
        ...
        game.set_fen_position("r4rk1/1p2Pppp/p7/2P1n3/8/B7/P4PPP/R4RK1 b KQkq - 0 1");
        ...
        assert_ne!(initial_hash, after_promotion_hash,
    ...
    fn test_en_passant_target_file_hash() {
        ...
        assert_ne!(initial_hash, with_ep_hash,
    ...
    fn test_multiple_moves_hash_consistency() {
        ...
        for uci_mv in &test_moves {
            ...
            assert_ne!(before_move_hash, after_move_hash,
                       "Hash should change after each move");

            board.unmake_move(&mv);
            assert_eq!(before_move_hash, board.hash,
    ...
    fn test_zobrist_structure_initialization() {
        ...
        for i in 0..4 {
            assert_ne!(zobrist.castling_rights[i], 0,
                       "Castling right {} should be non-zero", i);
        }

        for i in 0..8 {
            assert_ne!(zobrist.en_passant[i], 0,
                       "En passant file {} should be non-zero", i);
        }

        // Check pieces array
        for square in 0..64 {
            for piece in 0..12 {
                assert_ne!(zobrist.pieces[square][piece], 0,
                           "Piece at square {}, type {} should be non-zero", square, piece);
            }
        }
        ...
        for &val in &zobrist.castling_rights { values.insert(val); }
        for &val in &zobrist.en_passant { values.insert(val); }
        for square in 0..64 {
            for piece in 0..12 {
        ...
        assert_eq!(values.len(), expected_unique,
