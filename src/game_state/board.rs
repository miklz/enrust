...
impl ChessBoard {
    ...
    fn algebraic_to_internal(&self, algebraic_notation: &str) -> i16 {
        if let Some(square) = Self::notation_to_square(algebraic_notation) {
    ...
    fn parse_algebraic_move(&self, uci_notation: &str) -> Option<Move> {
        ...
        let from = self.map_inner_to_outer_board(Self::notation_to_square(&uci_notation[0..2])?);
        let to = self.map_inner_to_outer_board(Self::notation_to_square(&uci_notation[2..4])?);
...
mod pawn_tests {
    ...
    fn test_pawn_single_push() {
        ...
        let moves = game.board.generate_moves(Color::White);

        assert_eq!(moves.len(), 1);
        assert_eq!(game.board.move_to_uci(&moves[0]), "e3e4");
    }
    ...
    fn test_pawn_double_push_from_start() {
        ...
        let move_ucis: Vec<String> = moves.iter().map(|mv| game.board.move_to_uci(mv)).collect();

        assert!(move_ucis.contains(&"e2e3".to_string()));
        assert!(move_ucis.contains(&"e2e4".to_string()));
    }
}
...
mod promotion_tests {
    ...
    fn test_pawn_promotion_options() {
        ...
        let moves = game.board.generate_moves(Color::White);
        let pawn_moves: Vec<_> = moves.iter().filter(|mv| mv.piece.is_pawn()).collect();

        // Should have 4 promotion options
        assert_eq!(pawn_moves.len(), 4);
        let move_ucis: Vec<String> = pawn_moves
            .iter()
            .map(|mv| game.board.move_to_uci(mv))
            .collect();

        assert!(move_ucis.contains(&"e7e8q".to_string()));
        assert!(move_ucis.contains(&"e7e8r".to_string()));
        assert!(move_ucis.contains(&"e7e8b".to_string()));
        assert!(move_ucis.contains(&"e7e8n".to_string()));
    }
    ...
    fn test_pawn_promotion_with_capture() {
        ...
        let promotion_moves: Vec<String> = moves
            .iter()
    ...
    fn test_no_promotion_before_seventh_rank() {
        ...
        assert!(pawn_moves.iter().all(|mv| mv.promotion.is_none()));
    }
}
...
mod blocked_pawn_tests {
    ...
    fn test_pawn_blocked_by_friendly() {
        ...
        let e3_pawn_moves: Vec<_> = pawn_moves
            .iter()
            .filter(|mv| mv.from == game.board.algebraic_to_internal("e3"))
            .collect();

        assert_eq!(e3_pawn_moves.len(), 0);
    }
